...
- [ ] 1.1 CSS 變數全站替換（`text-gray-*` → `var(--color-text*)`）
- [ ] 1.1 `@theme inline` 補齊 error/success/warning/info/accent/text 顏色 tokens
- [ ] 2.1 Button hover 加 `-translate-y-0.5` + active `scale-[0.98]` + `motion-reduce:transform-none`
- [ ] 2.3 建立 motion duration 三級 token 並套用
- [ ] 2.4 Card hoverable 增加 translateY + cursor-pointer
- [ ] 6.1 Placeholder 顏色改為 `#7A7F88`（WCAG AA）
- [ ] 6.2 sm 按鈕 mobile 自動 `h-10`

### 🟡 批次 2：狀態回饋與空狀態（2-3 天）

- [ ] 5.1 引入 Sonner 或自建 Toaster，全站串起成功 / 錯誤提示
- [ ] 5.2 表單驗證改 onBlur
- [ ] 4.2 為 6 個主要空狀態畫面建立 `<EmptyState>` 元件
- [ ] 3.3 為每個 dashboard 子路由補 `loading.tsx` 骨架屏
- [ ] 8.1 Dashboard stats cards 色彩對齊品牌色階

### 🟢 批次 3：Onboarding 完整流程（3-5 天）

- [ ] 4.1 三步驟 Welcome Modal
- [ ] 4.1 Dashboard 首次登入 Next-step 卡片
- [ ] 4.1 進度徽章（4/4 關鍵設定）
- [ ] 4.3 AI 面試準備頁 + 情境 Tooltip
- [ ] 5.3 Resume / Cover Letter Autosave 指示
- [ ] Supabase migration: `profile.onboarding_completed_at`、`profile.target_role`

### 🔵 批次 4：動效精緻化（1-2 天）

- [ ] 3.1 啟用 Next.js 16 View Transitions
- [ ] 3.2 列表 stagger 進場
- [ ] 3.4 Interview radar chart 中心擴張動畫
- [ ] 5.4 AI 生成改串流輸出（Gemini `generateContentStream`）
- [ ] 9.2 面試完成慶祝過渡頁
...
- [ ] 7.1 側邊欄分組
- [ ] 7.2 Breadcrumb 元件
- [ ] 6.3 Input 加 focus-visible ring
- [ ] 6.4 Skip-to-content 連結
- [ ] 1.2 Logo 多版本（反白/單色/lockup）

### 🟣 批次 6（長期）：Dark mode / Cmd+K / 其他
